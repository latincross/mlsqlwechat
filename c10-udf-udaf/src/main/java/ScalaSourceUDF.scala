...
object ScalaSourceUDF {
  ...
  private def getFunctionReturnType(src: String, className: String, methodName: Option[String]): (Int, DataType) = {
      ...
      val method = ScriptCodeCompiler.getMethod(clazz, methodName.getOrElse("apply"))
  ...
  def generateFunction(src: String, className: String, methodName: Option[String], argumentNum: Int): AnyRef = {
      lazy val clazz = ScriptCodeCompiler.compileScala(ScriptCodeCompiler.prepareScala(src, className))
      lazy val instance = ScriptCodeCompiler.newInstance(clazz)
      lazy val method = ScriptCodeCompiler.getMethod(clazz, methodName.getOrElse("apply"))
    argumentNum match {
